                    ...
                    const invoiceService1 = result.map((value) => {
                      ...
                      return {
                        serviceName: `Tiền ${value.label} ${
                          value.inputValue ? `(Số ${value.label} cũ ${value.inputValue}` : ''
                        } ${value.outputValue ? `, Số ${value.label} mới ${value.outputValue})` : ''} `,
...
const createBillHouse = async (req, res, next) => {
  ...
  setTimeout(() => {
    if (billHa.length !== 0) {
      billHa.map((item) => {
        const { idRoom, month, year, invoiceService, paidAmount } = item;
        ...
        Bill.find({ idRoom: idRoom, month: month, year: year }).exec(async (err, data) => {
          await Bill.findOne({
            ...
            const { invoiceService } = data1;
            console.log('vcvcvcvcv', invoiceService && invoiceService.reduce((prev, curr) => prev + curr.amount, 0));
          ...
          //   { idRoom: idRoom, month: month == 12 ? 1 : month, year: month == 12 ? year + 1 : year },
...
const updateBill = async (req, res) => {
  ...
  await Bill.findOneAndUpdate({ _id: req.params.id }, { ...req.body }, async (err, data) => {
    ...
    console.log('gia', SumValue - paidAmount);
    console.log('thang', month == 12 ? '1' : (Number(month) + 1).toString());
    console.log('nam', month == 12 ? (Number(year) + 1).toString() : year);
