              ...
              Service.find({ idHouse: dataList.idHouse }).exec((err, docs) => {
                try {
                  const item = docs.map((value) => {
                    return { label: value.label, price: value.price };
                  });
                  ...
                  BillService.find({
                    ...
                    const invoiceService1 = result.map((value) => {
                      ...
                      return {
                        serviceName: `Tiền ${value.label} ${value.inputValue ? `(Số ${value.label} cũ ${value.inputValue}` : ''
...
const updateBill = async (req, res) => {
  await Bill.findOneAndUpdate({ _id: req.params.id }, { ...req.body }, (err, data) => {
    if (err || !data) {
...
module.exports = { createBillRoom, getBillAll, getBillId, read, removeBill, createBillHouse, updateBill, getBillIdRoom };
