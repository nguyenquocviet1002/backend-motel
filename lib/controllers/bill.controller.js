                  ...
                  dataRoom.map(async (item) => {
                    if (item.listMember.length) {
                      ...
                      try {
                        ...
                        const dataBillService = await Promise.all(
                          item.service.map(async (itemB) => {
                            try {
                              ...
                              console.log('tien nuoc', tienNuoc);
                              ...
                              const otherService = item.service.map((itemOtherService) => {
                                return {
                              ...
                              const map = otherService.reduce((t, v) => {
                                const { name, ...rest } = v;
                                t = rest;
                                return t;
                              }, {});
                              return [
                                {
                                  ...
                                  amount: tienDien.amount * (tienDien.outputValue - tienDien.inputValue),
                                },
                                {
                                  serviceName: 'Tiền Nước',
                                  amount: tienNuoc.amount * (tienNuoc.outputValue - tienNuoc.inputValue),
                                },
                                { ...map },
                                {
                        ...
                        //end call full service

                        return {
                          paymentStatus: 0,
                          paidAmount: 0,
                          debt: dataBill
                            ? dataBill.invoiceService.reduce((prev, curr) => prev + curr.amount, 0) + dataBill.debt
                            : 0,
                          name: item.name,
                          idRoom: item._id,
                          ...
                          idAuth: item.idAuth,
                          memberName: customerName,
                          invoiceService: dataBillService[0] || [],
                          idHouse: req.params.idHouse,
      ...
      listRoom.map(async (item) => {
        await Room.findById(item, async (err, dataRoom) => {
          ...
          if (dataRoom !== []) {
            ...
            const otherService = service.map((itemOtherService) => {
              return {
                serviceName: itemOtherService.label,
                amount: itemOtherService.price,
              };
            ...
            const dataBillService = [
              {
                serviceName: 'Tiền Điện',
                amount: tienDien ? tienDien.amount || 0 * (tienDien.outputValue - tienDien.inputValue) : 0,
              },
              {
                serviceName: 'Tiền Nước',
                amount: tienNuoc ? tienNuoc.amount || 0 * (tienNuoc.outputValue - tienNuoc.inputValue) : 0,
              },
              { ...map },
              {
                serviceName: 'Tiền Nhà',
            ...
            const resultData = new Bill({
              ...
              memberName: customerName,
              invoiceService: dataBillService,
              idHouse: idHouse,
              houseName: dataHouse.name,
              address: dataHouse.address,
              month: req.body.month,
