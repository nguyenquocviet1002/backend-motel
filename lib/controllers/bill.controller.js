    ...
    await Bill.findOne(
      ...
      (err, docs) => {
        if (docs == null) {
          ...
          console.log(
            'docs.invoiceService.reduce((prev, curr) => prev + curr.amount, 0) + docs.debt - docs.paidAmount',
            docs.invoiceService.reduce((prev, curr) => prev + curr.amount, 0) + docs.debt - docs.paidAmount,
          );
          newData.push({
            paymentStatus: false,
    ...
    await Bill.findOne(
      ...
      (err, docs) => {
        ...
        if (docs == null) {
          ...
          console.log(
            'docs.invoiceService.reduce((prev, curr) => prev + curr.amount, 0)',
            docs.invoiceService.reduce((prev, curr) => prev + curr.amount, 0),
          );
          newData.push({
            paymentStatus: false,
...
const updateBill = async (req, res) => {
  ...
  await Bill.findOneAndUpdate({ _id: req.params.id }, { ...req.body }, async (err, data) => {
    ...
    console.log('SumValue - paidAmount', req.body);
    if (err || !data) {
