...
const setAmount = async (req, res, next) => {
  ...
  await Service.find({ idHouse: data.idHouse }).exec((err, docs) => {
    try {
      ...
      BillService.find({ idRoom: data.idRoom, idHouse: data.idHouse, month: req.body.month, year: req.body.year }).exec(
        (err, data) => {
          if (data.length == 0) {
            res.status(400).json({
            ...
            const invoiceService = result.map((value) => {
              ...
              return {
                serviceName: `Tiền ${value.name} ${
                  value.inputValue ? `(Số ${value.name} cũ ${value.inputValue}` : ''
