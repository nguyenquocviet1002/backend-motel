                  ...
                  BillService.find({
                    ...
                    await billHa.map(async (itemRoom) => {
                      await Bill.findOneAndDelete(
                        ...
                        (err, docs) => console.log('docs', docs),
                      );
                    });
                  });
                } catch (err) {
...
const updateBill = async (req, res) => {
  try {
    const statusBill = req.body.SumValue - req.body.paidAmount > 0 ? false : true;

    await Bill.findOneAndUpdate(
      { _id: req.params.id },
      { paymentStatus: statusBill, ...req.body },
      async (err, data) => {
        ...
        if (err || !data) {
          ...
          console.log('data', { paymentStatus: statusBill, ...req.body });
