    ...
    if (billHa.length !== 0) {
      billHa.map((item) => {
        ...
        Bill.findOne({
          ...
          console.log(idRoom, typeof month, typeof year);
          if (data1 == null) {
            Bill.insertMany(item);
          } else {
            await Bill.findByIdAndDelete(data1._id, (err, docs) => {});
            await Bill.insertMany(item, async (err, docs) => {
              ...
              await Bill.find({
                ...
                const { invoiceService, paidAmount, debt } = item;
                const newItem = {
                  ...
                  month: docs.month,
                  year: docs.year,
                  memberName: docs.memberName,
                  invoiceService: docs.invoiceService,
