...
export const createAllBillForHouse = async (req, res) => {
  ...
  const newData = data.map((data) => {
    return { ...data, idHouse, name, month, year, price, unit, amount: (data.outputValue - data.outputValue) * price };
  });

  BillService.insertMany(newData)
    .then((data) => {
      return res.status(200).json({
        data,
      });
...
export const getAllBillByMonthYear = async (req, res) => {
  ...
  if (!idHouse || !month || !year) {
    ...
    await BillService.find({ idHouse, month, year, name: type }, async (err, docs) => {
      ...
      if (docs.length == 0) {
        if (month > 1) {
          await BillService.find({ name: type, idHouse, month: month - 1, year }, (err, docs) => {
            if (err) {
              return res.status(400).json({
                err,
              });
        ...
        if (month <= 1) {
          await BillService.find({ name: type, idHouse, month: 12, year: year - 1 }, (err, docs) => {
            if (err) {
              return res.status(400).json({
                err,
              });
