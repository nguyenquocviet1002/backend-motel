      ...
      user.save(err => {
        ...
        token.save(err => {
          ...
          smtpTransport.sendMail(data, err => {
            ...
            return res.status(200).json({ message: 'Email xác minh đã được gửi tới ' + user.email + '. Vui lòng kiểm tra email của bạn để xác minh.' });
          });
        })
      })
    }
...
exports.confirmEmail = (req, res) => {
  Token.findOne({ token: req.params.token }, (err, token) => {
    if (!token) {
      return res.status(401).json({ message: 'Liên kết xác minh của bạn đã hết hạn. Vui lòng nhấp vào gửi lại để xác minh email của bạn' });
    }
    else {
...
exports.resendLink = (req, res) => {
  User.findOne({ email: req.body.email }, (err, user) => {
    if (!user) {
      return res.status(400).json({ message: "Chúng tôi không thể tìm thấy người dùng có email đó. Đảm bảo rằng Email của bạn là chính xác" })
    }
    else if (user.isVerified) {
      return res.status(200).json({ message: "Người dùng đã được xác minh. Vui lòng hãy đăng nhập" });
    }
