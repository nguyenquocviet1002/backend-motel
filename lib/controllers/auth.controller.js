...
exports.forgot_password = (req, res) => {
  async.waterfall([
    ...
    function (user, token, done) {
      User.findByIdAndUpdate({ _id: user._id }, { reset_password_token: token, reset_password_expires: Date.now() + 86400000 }, { new: true }).exec((err, new_user) => {
    ...
    function (token, user, done) {
      const data = {
        ...
        context: {
          url: 'http://localhost:4000/auth/reset-password?token=' + token,
          name: user.name
        }
      }
...
exports.reset_password = (req, res, next) => {
  User.findOne({ reset_password_token: req.body.token, reset_password_expires: { $gt: Date.now() } }).exec((err, user) => {
    if (!err && user) {
