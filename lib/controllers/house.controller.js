...
const Service = require('../models/house.model');
...
const removeService = async (req, res) => {
  ...
  //   let data = { service: house };
  //   if (req.params.idHouse) {
  await Service.findByIdAndDelete(req.body.idService, (err, docs) => {
    if (err) {
      return res.status(400).json({
        error: 'dich vu không tồn tại',
      });
    } else {
      return res.status(200).json({
        error: 'xoa dịch vụ thành công',
        docs: docs,
      });
    }
  });
...
module.exports = { create, list, houseByID, read, remove, update, addService, houseByIDService, removeService };
