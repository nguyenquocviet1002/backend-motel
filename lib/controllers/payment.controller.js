...
const vnp_ReturnUrl = 'http://localhost:4000/manager/ternant/payment/return-payment';
  ...
  if (secureHash === signed) {
    ...
    try {
      ...
      const newPayment = new Payment({
        ...
        content: `Nhà ${newDataPayment[5]} đã thanh toán số tiền là ${newDataPayment[2]} cho hóa đơn tháng ${newDataPayment[0]} năm ${newDataPayment[1]} `,
      });

      await newPayment.save((err, data) => {
        if (err || !data) {
          console.log('khong luu dc');
        } else {
          console.log('luu payment thanh cong');
        }
      });
    } catch (error) {}
  } else {
