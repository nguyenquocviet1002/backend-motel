...
export const checkIdBooking = (req, res, next) => {
  ...
  if (!req.params.idBooking.match(/^[0-9a-fA-F]{24}$/)) {
    return res.status(400).json({
      error: 'id đặt trước không đúng định dạng',
    });
  } else {
    next();
  }
...
export const checkYearParam = (req, res, next) => {
  if (!req.params.year) {
    return res.status(400).json({
      error: 'Thiếu năm để lấy dữ liệu!',
    });
  }

  if (req.params.year < 1) {
    return res.status(400).json({
      error: 'Nhập năm không hợp lệ!',
    });
  } else {
    next();
  }
...
export const checkMonthParam = (req, res, next) => {
  if (!req.params.month) {
    return res.status(400).json({
      error: 'Thiếu tháng để lấy dữ liệu!',
    });
  }

  if (req.params.year < 1) {
    return res.status(400).json({
      error: 'Nhập tháng không hợp lệ!',
    });
  } else {
    next();
  }
